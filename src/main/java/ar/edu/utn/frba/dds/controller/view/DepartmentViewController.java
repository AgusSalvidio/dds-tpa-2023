...
public class DepartmentViewController {
  ...
  public void index(Context context) {
    ...
    model.put("object-list", this.applicationContext
        .departmentManagementSystem().objectList(Department.class.getName()));
  ...
  public void edit(Context context) throws Exception {
    ...
    Department toEdit = this.applicationContext.departmentManagementSystem().departmentById(id);
    model.put("user", this.applicationContext.loggedUser(context));
    model.put("title", pageTitle);
    model.put("action", actionString);
  ...
  public void update(Context context) throws Exception {
    Map<String, Object> model = new HashMap<>();
    Integer id = Integer.parseInt(context.pathParam("id"));
    Department toUpdate = this.applicationContext.departmentManagementSystem().departmentById(id);
    model.put("name", context.formParam("name"));
    this.applicationContext.departmentManagementSystem().updateFromModel(toUpdate, model);
  ...
  public void delete(Context context) throws Exception {
    Integer id = Integer.parseInt(context.pathParam("id"));
    Department toDelete = this.applicationContext.departmentManagementSystem().departmentById(id);
    this.applicationContext.departmentManagementSystem().stopManaging(toDelete);
    context.redirect(actionString);
  }
